...
import com.anbetter.album.models.ad.AdViewHolder;
...
public class PhotosAdapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof PhotoViewHolder) {
            final PhotoInfo item = (PhotoInfo) dataList.get(p);
            if (item == null) return;
    ...
    private void singleSelector(PhotoInfo photoInfo, int position) {
        if (!Result.isEmpty()) {
            if (Result.getPhotoPath(0).equals(photoInfo.path) && !Setting.singleCheckedBack) {
    ...
    private void updateSelector(TextView tvSelector, boolean selected, PhotoInfo photoInfo, int position) {
        if (selected) {
            String number = Result.getSelectorNumber(photoInfo);
            if (number.equals("0")) {
