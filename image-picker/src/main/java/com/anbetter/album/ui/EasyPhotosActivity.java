...
import com.anbetter.album.models.ad.AdListener;
...
import com.anbetter.album.ui.adapter.AlbumItemsAdapter;
import com.anbetter.album.ui.adapter.PhotosAdapter;
import com.anbetter.album.ui.widget.PressedTextView;
import com.anbetter.album.utils.color.ColorUtils;
import com.anbetter.album.utils.media.MediaScannerConnectionUtils;
import com.anbetter.album.utils.media.MediaUtils;
import com.anbetter.album.utils.permission.PermissionUtil;
import com.anbetter.album.utils.settings.SettingsUtils;
import com.anbetter.album.utils.system.SystemUtils;
import com.anbetter.album.utils.uri.UriUtils;
...
public class EasyPhotosActivity extends AppCompatActivity implements AlbumItemsAdapter
    ...
    private ArrayList<PhotoInfo> resultList = new ArrayList<>();
    ...
    private void hasPermissions() {
        ...
        if (Setting.selectedPhotoInfos.size() > Setting.count) {
            throw new RuntimeException("AlbumBuilder: 默认勾选的图片张数不能大于设置的选择数！" + "|默认勾选张数：" + Setting.selectedPhotoInfos.size() + "|设置的选择数：" + Setting.count);
        }
        ...
        if (!Setting.selectedPhotoInfos.isEmpty()) {
            for (PhotoInfo selectedPhotoInfo : Setting.selectedPhotoInfos) {
                if (TextUtils.isEmpty(selectedPhotoInfo.name)) {
                    albumModel.fillPhoto(this, selectedPhotoInfo);
                }
                selectedPhotoInfo.selectedOriginal = Setting.selectedOriginal;
                Result.addPhoto(selectedPhotoInfo);
            }
        }
    }
            ...
            case RESULT_OK:
                ...
                if (Code.REQUEST_PUZZLE_SELECTOR == requestCode) {
                    try {
                        ...
                        addNewPhoto(applicationName, puzzlePhotoInfo);
    ...
    private void startCrop(AppCompatActivity context, PhotoInfo photoInfo, Intent data) {

        String source = photoInfo.path;
...
//        String suffix = source.substring(source.lastIndexOf("."));
        String imageName = "IMG_CROP_%s" + getImageSuffix(photoInfo.type);
    ...
    private void addNewPhoto(String albumName, PhotoInfo photoInfo) {
        MediaScannerConnectionUtils.refresh(this, photoInfo.path);
        photoInfo.selectedOriginal = Setting.selectedOriginal;

        String albumItem_all_name = albumModel.getAllAlbumName(this);
        albumModel.album.getAlbumItem(albumItem_all_name).addImageItem(0, photoInfo);
        if (albumName == null) {
            final File parentFile = new File(photoInfo.path).getParentFile();
            albumName = parentFile.getName();
        }
        albumModel.album.addAlbumItem(albumName, photoInfo.path);
        albumModel.album.getAlbumItem(albumName).addImageItem(0, photoInfo);
        ...
        if (Setting.count == 1) {
            Result.clear();
            int res = Result.addPhoto(photoInfo);
            onSelectError(res);
        } else {
    ...
    private void onCameraResult(File file) {
        ...
        final Pair<String, PhotoInfo> pair = MediaUtils.getPhoto(this, file);
        if (pair == null || pair.second == null) {
            throw new RuntimeException("EasyPhotos拍照保存的图片不存在");
        }

        String bucketName = pair.first;
        PhotoInfo photoInfo = pair.second;
        ...
        addNewPhoto(bucketName, photoInfo);
    }
    ...
    private void done() {
        ...
        resultList.clear();
        resultList.addAll(Result.photoInfos);
        intent.putParcelableArrayListExtra(EasyPhotos.RESULT_PHOTOS, resultList);
        ArrayList<String> resultPaths = new ArrayList<>();
        for (PhotoInfo photoInfo : resultList) {
            if (!TextUtils.isEmpty(photoInfo.compressPath)) {
                resultPaths.add(photoInfo.compressPath);
            } else if (!TextUtils.isEmpty(photoInfo.cropPath)) {
                resultPaths.add(photoInfo.cropPath);
            } else {
                resultPaths.add(photoInfo.path);
            }
        }
    ...
    private void compress() {
        Setting.compressEngine.compress(this, resultList, new CompressCallback() {
            ...
            @Override
            public void onSuccess(ArrayList<PhotoInfo> photoInfos) {
                showProgress(false);
                done();
            }

            @Override
            public void onFailed(ArrayList<PhotoInfo> photoInfos, final String message) {
                showProgress(false);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
